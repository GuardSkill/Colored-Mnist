...
from models import ResNet_model, CNN, GResNet_model, GramRelationMax
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gamma', type=float, default=0.1,
                        help='coefficient for Gram matrix loss')
    ...
    if args.load_dir == '':
        ...
        if args.arch == 'resnet':
            model0 = GResNet_model(bn=args.bn, num_classes=nb_classes, depth=args.depth, \
...
def train(epoch):
    ...
    correct = 0
    total = 0
    total_loss, tot_regularization_loss,tot_gram_loss = 0, 0,0
    gram=GramRelationMax()
    ...
    for batch_idx in tqdm.tqdm(range(tot_iters), total=tot_iters):
        ...
        feature_list, outputs, hid_repr=(model(inputs, ret_hid=True))
        #
        ...
        gram_loss=gram(feature_list[7],feature_list[8])
        tot_gram_loss=tot_gram_loss+gram_loss

        tot_regularization_loss = tot_regularization_loss + regularization_loss.data

        total_loss_ = loss + args.beta * regularization_loss +args.gamma *gram_loss
    ...
    return total_loss / (batch_idx + 1), acc, tot_regularization_loss / (batch_idx + 1),tot_gram_loss/(batch_idx + 1)


optimizer = torch.optim.Adam(params, lr=args.lr, weight_decay=args.wdecay)

best_acc, best_loss = 0, np.inf
train_loss_list, train_acc_list, valid_acc_list, test_acc_list, reg_loss_list,gram_loss_list = [], [], [], [], [], []
epoch = 0
...
def train_fn():
    ...
    while epoch < args.epochs:
        ...
        loss, train_acc, regularization_loss,gram_loss = train(epoch)
        ...
        status = 'Epoch {}/{} | Loss {:3.4f} | acc {:3.2f} | test-acc {:3.2f} | reg_loss : {:3.4f} | gram_loss : {:3.4f}'. \
            format(epoch, args.epochs, loss, train_acc, test_acc, regularization_loss,gram_loss)
        print(status)
